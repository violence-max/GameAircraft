 ...
 * @author hitsz
 */
public abstract class AbstrcatFlyingObject {
    ...
    public AbstrcatFlyingObject() {
    }

    public AbstrcatFlyingObject(int locationX, int locationY, int speedX, int speedY) {
     ...
     *
     * @param abstrcatFlyingObject 撞击对方
     * @return true: 我方被击中; false 我方未被击中
     */
    public boolean crash(AbstrcatFlyingObject abstrcatFlyingObject) {
        // 缩放因子，用于控制 y轴方向区域范围
        int factor = this instanceof AbstractAircraft ? 2 : 1;
        int fFactor = abstrcatFlyingObject instanceof AbstractAircraft ? 2 : 1;

        int x = abstrcatFlyingObject.getLocationX();
        int y = abstrcatFlyingObject.getLocationY();
        int fWidth = abstrcatFlyingObject.getWidth();
        int fHeight = abstrcatFlyingObject.getHeight();
